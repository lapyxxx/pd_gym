...
export const config = {
  ...
  maxConcurrentTasks: Number(process.env.MAX_CONCURRENT_TASKS ?? "4"),
  threadWorkspacesDir: resolveFromRoot(process.env.THREAD_WORKSPACES_DIR ?? "../thread-workspaces"),
};

fs.mkdirSync(path.dirname(config.dbPath), { recursive: true });
fs.mkdirSync(config.uploadsDir, { recursive: true });
fs.mkdirSync(config.threadWorkspacesDir, { recursive: true });
